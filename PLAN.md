...
**3b (done 2026-09-17, committed as `dc993d6`):**
...
- **Gone:** the `onElectionChanged`/`onElectionDeleted`/`sendLinksReminder` props, the React contexts `factoryContext.ts`/`antiFactoryContext.ts`, and their providers in `entry.client.tsx`.
- **`Abode`** creates elections in a `clientAction` that returns `redirect(…#token)`; its modal submits via React Router's `<Form method="post">`.
...
- **Verified:**
  ...
  - against the dev server: smoke 14/15 (the known flaw in the test), encoding 5/5, integration 13/13, tokens 5/5, actions 3/3
